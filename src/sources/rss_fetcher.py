...
def fetch_google_alerts(alerts_config: List[dict], max_age_days: int = 7) -> List[Article]:
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(future_to_alert):
            ...
            try:
                fetched_articles = future.result()
                articles.extend(fetched_articles)
            except Exception as e:
                logger.error(f"Error in fetching thread for {alert.get('name', 'Unknown')}: {e}")
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(future_to_feed):
            ...
            try:
                ...
                logger.error(f"Error in fetching thread for {feed_config.get('name', 'Unknown')}: {e}")
