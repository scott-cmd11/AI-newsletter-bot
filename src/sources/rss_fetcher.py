...
def _fetch_single_google_alert(alert_config: dict, cutoff_date: datetime) -> List[Article]:
    """
    Helper function to fetch a single Google Alert feed.
    
    Args:
        alert_config: Google Alert configuration dictionary
        cutoff_date: Date cutoff for articles
        
    Returns:
        List of Article objects found in this feed
    """
    articles = []
    url = alert_config.get('url', '')
    if not url:
        logger.warning(f"Skipping '{alert_config.get('name', 'Unknown')}' - no URL configured")
        print(f"  ⚠️  Skipping '{alert_config.get('name', 'Unknown')}' - no URL configured")
        return articles

    alert_name = alert_config.get('name', url[:50])
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            article = Article(
                ...
                url=entry.get('link', ''),
                source=alert_config.get('name', 'Google Alerts'),
                published=pub_date,
...
def fetch_google_alerts(alerts_config: List[dict], max_age_days: int = 7) -> List[Article]:
    ...
    # Limit max workers to avoid overwhelming the network or being rate limited
    max_workers = min(len(alerts_config), 5)

    if max_workers == 0:
        return []

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Create a partial function with cutoff_date
        fetch_func = partial(_fetch_single_google_alert, cutoff_date=cutoff_date)

        # Execute in parallel
        results = executor.map(fetch_func, alerts_config)
...
def fetch_rss_feeds(feeds_config: List[dict], max_age_days: int = 7) -> List[Article]:
    ...
    # Use ThreadPoolExecutor for parallel fetching
    # Limit max workers to avoid overwhelming the network or being rate limited
    max_workers = min(len(feeds_config), 5)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Create a partial function with cutoff_date
        fetch_func = partial(_fetch_single_rss_feed, cutoff_date=cutoff_date)

        # Execute in parallel
        results = executor.map(fetch_func, feeds_config)
