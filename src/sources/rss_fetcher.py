...
def _fetch_single_google_alert(alert: dict, cutoff_date: datetime) -> List[Article]:
    ...
    Args:
        alert: Google Alert configuration with 'url', 'name', 'priority'
        cutoff_date: Cutoff date for article age
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(future_to_alert):
            try:
                ...
                logger.error(f"Error processing Google Alert {alert_name}: {e}")
...
def fetch_rss_feeds(feeds_config: List[dict], max_age_days: int = 7) -> List[Article]:
    ...
    with ThreadPoolExecutor() as executor:
        future_to_feed = {
            executor.submit(_fetch_single_rss_feed, feed_config, cutoff_date): feed_config
        ...
        for future in as_completed(future_to_feed):
            try:
                ...
                logger.error(f"Error processing RSS feed {feed_name}: {e}")
