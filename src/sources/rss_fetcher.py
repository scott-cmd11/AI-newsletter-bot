    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                logger.error(f"Error in Google Alert fetch thread: {e}")

    return articles


def _fetch_single_rss(feed_config: dict, cutoff_date: datetime) -> List[Article]:
...
def fetch_rss_feeds(feeds_config: List[dict], max_age_days: int = 7) -> List[Article]:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(_fetch_single_rss, feed_config, cutoff_date)
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                logger.error(f"Error in RSS feed fetch thread: {e}")
